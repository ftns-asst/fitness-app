...
> Связанные документы: `fitness-assistant-frontend-design.mk` (спецификация),
...
1. Точные схемы ответов: `user`, `tokens` (имена полей, `expires_in` /
   `expires_at`, ротация refresh), `profile` (имена полей, единицы, enum пола).
2. Способ передачи access_token (`Authorization: Bearer`?), обязательные
   заголовки.
3. Base URL окружений (dev/staging/prod), тестовые учётки, рателимиты.
4. `POST /auth/login` при несуществующем email: 404 `undefined_error` или
   отдельный key (`email_not_found`)? Зафиксировать UX-решение.
5. Правила пароля: точный набор спецсимволов, обязательные классы символов,
   ключ ошибки при невалидном пароле.
6. Refresh-семантика: одноразовость (ротация), поведение при повторном
   использовании. Критично для single-flight.
...
9. Подтверждение, что параметры тела не являются частью `signup`.
...
- **Хранение refresh-токена**: SQLite в приватном каталоге приложения
  (Android internal storage). Android Keystore через JNI — backlog.
...
- **Mock-API повторяет ключи ошибок** бэкенда, поэтому UI входа не
  переписывается при переходе от mock к реальному API; расхождения контракта
  закрываются слоем DTO/маппинга (`AApi_Error`, структуры DTO).
