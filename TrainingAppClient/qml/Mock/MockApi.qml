...
QtObject {
    ...
    function startDemoSession() {
        const record = api.usersByEmail["igor@example.com"];
        const tokens = issueTokenPair(record.user.id);

        return MockSession.authenticate(record.user, record.profile, tokens);
    }
